...
pickle_file = "/home/pi/Desktop/facerecognitionsystem-backend/fr-pickle/datasets.pickle"
...
try:
    ...
    o.write(pickle.dumps({"face": None, "name": None}))
    o.close()
...
for every_user in users:
    ...
    for everyuser_img in list_img:
        ...
        if (name not in dataset_face_names):
            img_file = face_recognition.load_image_file(everyimg_path)
            boxes = face_recognition.face_locations(img_file, model="hog")
            img_encoding = face_recognition.face_encodings(img_file, boxes)[0]
...
datasets = {"face": dataset_face_encodings, "name": dataset_face_names}
p = open("/home/pi/Desktop/facerecognitionsystem-backend/fr-pickle/pickle/datasets.pickle", "wb")
p.write(pickle.dumps(datasets))
p.close()
